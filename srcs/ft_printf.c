...
/*   Updated: 2019/07/17 15:06:25 by ariperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		put_o_u_x(t_printf *p)
{
	...
	p->a.sign = (p->a.p & (HASH | HASH1) ? 2 : (p->a.p & (HASH | HASH0) ? 1 : 0));
	p->a.sign = (p->a.p & HASH ? p->a.sign : 0);
	p->a.zeros = MAX(p->a.precision - (int)ft_strlen(p->a.arg), 0);
	p->a.str = (int)ft_strlen(p->a.arg);
	p->a.space = MAX(p->a.width - ((int)ft_strlen(p->a.arg) + p->a.zeros + p->a.sign), 0);
	total = p->a.zeros + p->a.str + p->a.space;
	while (!(p->a.p & MINUS) && p->a.precision >= 0 && p->a.space--)
		write(1, " ", 1);
	...
	while (p->a.zeros--)
		write(1, "0", 1);
	while (p->a.p & ZERO && p->a.precision == -1 && p->a.space--)
		write(1, "0", 1);
	write(1, p->a.arg, ft_strlen(p->a.arg));
	while (p->a.p & MINUS && p->a.space--)
		write(1, " ", 1);
...
int		speci_o_u_x(t_printf *p)
{
	...
	p->a.arg = ft_strdup(itoa_printf(o_u_x, p));
	if (p->a.p & HASH0 && !(p->a.p & HASH1))
		p->a.arg = ft_convert_base(p->a.arg, base(p->a.arg), "01234567");
	else if (!(p->a.p & HASH0) && p->a.p & HASH1)
		p->a.arg = ft_convert_base(p->a.arg, base(p->a.arg), "0123456789abcdef");
	else if (p->a.p & (HASH0 | HASH1))
		p->a.arg = ft_convert_base(p->a.arg, base(p->a.arg), "0123456789ABCDEF");
	if (o_u_x == 0 && p->a.precision == 0)
		p->a.arg = ft_strdup("");
	return (put_o_u_x(p));
}
